...
RegisterCustomEventListener("check_perks_for_players", function(data)
	...
	for _, playerId in pairs(_G.VisiblePerksForEnemyTeam[playerTeam]) do
		if _G.PlayersPatreonsPerk[playerId] then
			local perkName = _G.PlayersPatreonsPerk[playerId]:gsub("_t%d*", "_t0")
			CustomGameEventManager:Send_ServerToPlayer(PlayerResource:GetPlayer(data.PlayerID), "show_player_perk", { playerId = playerId, perkName = perkName})
		end
	end
end)
